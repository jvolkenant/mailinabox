...
sed -i "s^define('TIMEZONE', .*^define('TIMEZONE', '$(readlink /etc/localtime | sed 's#^.*zoneinfo/##')');^" /usr/local/lib/z-push/config.php
...
sed -i "s^define('TIMEZONE', .*^define('TIMEZONE', '$(readlink /etc/localtime | sed 's#^.*zoneinfo/##')');^" /usr/local/lib/z-push/autodiscover/config.php
